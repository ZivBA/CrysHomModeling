...
public class EvaluateBatchOfProteins extends MeshiProgram implements Residues, AtomTypes {
...
//					tmpList = getMatchingAtoms(reference,	new AtomList(pathSecondEval + "/" + models[i]+"_KB-nH_bf.pdb"));
//					tmpList = getMatchingAtoms(reference,	new AtomList(pathSecondEval + "/" + models[i]+".noRottenMinimialSCMOD.min.pdb"));
					if (tmpList!=null) { 		
						...
						System.out.println(tmpStr.replace('2','4'));
...
//						for (int rr=0 ; rr<model.residues().size() ; rr++) 
//						if (!model.residues().residueAt(rr).dummy()) {
//							double sum = 0.0;
//							for (int cc=0 ; cc<model.residues().residueAt(rr).atoms().size() ; cc++)
//								sum+= model.residues().residueAt(rr).atoms().atomAt(cc).energy();
//							System.out.println(model.residues().residueAt(rr).number + " " + sum);
//						}
					}
				}
			}
