...
public class MergeLoopsToModel extends MeshiProgram implements Residues, AtomTypes {
	...
	public static void main(String[] args) {
		...
		try {
			initial = new ExtendedAtomsProtein(args[0].trim(),DO_NOT_ADD_ATOMS);
			System.out.println("# Initial model is: "+args[0].trim());
			...
			String errorMessage = ("\n                  ******************\n"+
					"Usage: java -Xmx300m MergeLoopsToModel <initial  model> <output filename> <loop #1 pdb> <loop #1 from res.> <loop #1 to res.> <loop #2 pdb> <loop #2 from res.> <loop #2 to res.>... \n"+
			"                    ******************\n");
		...
		// Writing to file
		try {
			output.atoms().backbone().print(new MeshiWriter(outputFileName));
		}
		catch (Exception e) {
...
//		// Now changing the sidechains near the loops
//		for (int loop=0 ; loop<loops.size() ; loop++){
//			for (int res=froms.get(loop).intValue() ; res<=tos.get(loop).intValue() ; res++) {
//				for (int atomC=0 ; atomC<loops.get(loop).residue(res).atoms().size() ; atomC++) {
//					Atom atom = loops.get(loop).residue(res).atoms().atomAt(atomC);
//					for (int c=0 ; c<output.atoms().size() ; c++) {
//						Atom otherAtom = output.atoms().atomAt(c);
//						if (((otherAtom.residueNumber()<froms.get(loop).intValue()) ||
//								(otherAtom.residueNumber()>tos.get(loop).intValue())) &&
//								!otherAtom.isBackbone &&
//								(atom.distanceFrom(otherAtom)<4.0)) {  // this atom is close to the loop
//							for (int cc=0; cc<output.residue(otherAtom.residueNumber()).atoms().size() ; cc++) {
//								Atom stillOtherAtom = loops.get(loop).residue(otherAtom.residueNumber()).atoms().findAtomInList(
//										output.residue(otherAtom.residueNumber()).atoms().atomAt(cc).name(), 
//										output.residue(otherAtom.residueNumber()).atoms().atomAt(cc).residueNumber());
//								output.residue(otherAtom.residueNumber()).atoms().atomAt(cc).setXYZ(
//										stillOtherAtom.x(), stillOtherAtom.y(), stillOtherAtom.z());
//							}
//						}
//					}
//				}
