...
public class EvaluateBatchOfCoarseLoops extends MeshiProgram implements Residues, AtomTypes {
	...
	public static void main(String[] args) throws MinimizerException, LineSearchException {
		double lowRms = 1.0;
		double highRms = 2.0;
		double rangeRms = 0.25;
		int[][] hist6 = new int[190][101];
		int[][] hist12 = new int[190][101];
		init(args); 
...
//				new CompositePropensity2DCreator(1.0),
		...
		for (int res=0 ; res<reference.residues().size() ; res++) {
			if (!reference.residues().residueAt(res).dummy())
				RotamericTools.putCBinSCcenter(reference.residues().residueAt(res));
		}
...
//		TorsionValEnergy torsionTerm1 = (TorsionValEnergy) energy.getEnergyTerm(new TorsionValEnergy());
		...
		// histogram arrays
		double[] sumHigh6 = new double[model.atoms().size()];
		double[] numHigh6 = new double[model.atoms().size()];
		double[] sumLow6 = new double[model.atoms().size()];
		double[] numLow6 = new double[model.atoms().size()];
		double[] sumHigh12 = new double[model.atoms().size()];
		double[] numHigh12 = new double[model.atoms().size()];
		double[] sumLow12 = new double[model.atoms().size()];
		double[] numLow12 = new double[model.atoms().size()];
		double rmsVal = -999.0;
...
//						}
					...
					System.out.print("999999 " + i + " " + calcRMS(model, reference, resStart, resEnd) + " " +
							calcRMSallHeavyAtoms(model, reference, resStart, resEnd) + " " + energy.report(2) + " ");
					rmsVal = calcRMSallHeavyAtoms(model, reference, resStart, resEnd);
				}
				...
				if (Math.abs(rmsVal-lowRms)<rangeRms) {
					energy.resetAtomEnergies();
					cb_060.evaluate(true);
					for (int ccc=0; ccc<model.atoms().size() ; ccc++) {
						sumLow6[ccc] += model.atoms().atomAt(ccc).energy();
						numLow6[ccc]++;
					}
					energy.resetAtomEnergies();
					cb_120.evaluate(true);
					for (int ccc=0; ccc<model.atoms().size() ; ccc++) {
						sumLow12[ccc] += model.atoms().atomAt(ccc).energy();
						numLow12[ccc]++;
					}					
				}
				if (Math.abs(rmsVal-highRms)<rangeRms) {
					energy.resetAtomEnergies();
					cb_060.evaluate(true);
					for (int ccc=0; ccc<model.atoms().size() ; ccc++) {
						sumHigh6[ccc] += model.atoms().atomAt(ccc).energy();
						numHigh6[ccc]++;
					}
					energy.resetAtomEnergies();
					cb_120.evaluate(true);
					for (int ccc=0; ccc<model.atoms().size() ; ccc++) {
						sumHigh12[ccc] += model.atoms().atomAt(ccc).energy();
						numHigh12[ccc]++;
					}					
				}				
			}
		...
		for (int ccc=0; ccc<model.atoms().size() ; ccc++) {
			double aveHigh6 = sumHigh6[ccc]/(numHigh6[ccc]+1e-10);
			double aveHigh12 = sumHigh12[ccc]/(numHigh12[ccc]+1e-10);
			double aveLow6 = sumLow6[ccc]/(numLow6[ccc]+1e-10);
			double aveLow12 = sumLow12[ccc]/(numLow12[ccc]+1e-10);
			double diff6 = aveHigh6-aveLow6;
			double diff12 = aveHigh12-aveLow12;
			if (Math.abs(diff6)<4.9) {
				int ind6 = (int) Math.round(diff6*10.0 + 50.0);
				hist6[model.atoms().atomAt(ccc).type][ind6]++;
			}
			if (Math.abs(diff12)<4.9) {
				int ind12 = (int) Math.round(diff12*10.0 + 50.0);
				hist12[model.atoms().atomAt(ccc).type][ind12]++;
			}			
		}
		...
		for (int c=0 ; c<190 ; c++) {
			...
			for (int cc=0 ; cc<101 ; cc++)
				System.out.print(hist6[c][cc] + " ");
			System.out.println();
			System.out.print("777777 ");
			for (int cc=0 ; cc<101 ; cc++)
				System.out.print(hist12[c][cc] + " ");
			System.out.println();
		}
