...
//			DSSP dssp = new DSSP(dirName+"/Template.dssp");
			int[][] tmpMapping;
			boolean[] matchingRes = new boolean[2*solution.residues().size()];
			double[] resenergies = new double[2*solution.residues().size()];
			for (int c=0; c<resenergies.length ; c++)
				resenergies[c] = 0.0;
			double[] numenergies = new double[2*solution.residues().size()];
			for (int c=0; c<resenergies.length ; c++)
				numenergies[c] = 1e-10;
			boolean[] participatingRes = new boolean[2*solution.residues().size()];
			...
			EnergyCreator[] energyCreators = {new ROT1SolvationCreator(1.0,"5.0",false,matchingRes),
					new ROT1SolvationCreator(1.0,"6.0",false,matchingRes),
					new ROT1SolvationCreator(1.0,"7.0",false,matchingRes),
					new ROT1SolvationCreator(1.0,"8.0",false,matchingRes),
					new CBSolvationCreator(1.0,"5.0",false,matchingRes),
					new CBSolvationCreator(1.0,"6.0",false,matchingRes),
					new CBSolvationCreator(1.0,"7.0",false,matchingRes),
					new CBSolvationCreator(1.0,"8.0",false,matchingRes),
					new CentroidSolvationCreator(1.0,"5.0",false,matchingRes),
					new CentroidSolvationCreator(1.0,"6.0",false,matchingRes),
					new CentroidSolvationCreator(1.0,"7.0",false,matchingRes),
					new CentroidSolvationCreator(1.0,"8.0",false,matchingRes)};
			...
			for (int ali=0 ; ali<probA.alignmentNum()+2 ; ali++) {
				...
				if (ali==probA.alignmentNum()) {
					...
					tmpMapping = TrivialHomologyModeling.alignmentMapping(templateStart,
							psiblastAlignment[0], psiblastAlignment[1]);
				}
				...
				for (int c=0; c<participatingRes.length ; c++)
					participatingRes[c] = false;
...
//				int ssIsHE = 0;
//				int ssIsCoil = 0;
//				for (int c=0; c<homology.residues().size() ; c++) 
//					if (!homology.residues().residueAt(c).dummy() && 
//							tmpMapping[homology.residues().residueAt(c).number]!=null)
//						if ((dssp.SSofRes(tmpMapping[homology.residues().residueAt(c).number][0])=='E') ||
//								(dssp.SSofRes(tmpMapping[homology.residues().residueAt(c).number][0])=='H')) {
//							matchingRes[homology.residues().residueAt(c).number] = true;
//							ssIsHE++;
//						}
//						else {
//							ssIsCoil++;
//						}
//				System.out.println("The coil is " + (ssIsCoil*100.0/(ssIsCoil+ssIsHE)) + "% and the HE is " + (ssIsHE*100.0/(ssIsCoil+ssIsHE)) + "%");
				
				((ROT1SolvationCreator) energyCreators[0]).setMatchingResidues(matchingRes);
				((ROT1SolvationCreator) energyCreators[1]).setMatchingResidues(matchingRes);
				((ROT1SolvationCreator) energyCreators[2]).setMatchingResidues(matchingRes);
				((ROT1SolvationCreator) energyCreators[3]).setMatchingResidues(matchingRes);
				((CBSolvationCreator) energyCreators[4]).setMatchingResidues(matchingRes);
				((CBSolvationCreator) energyCreators[5]).setMatchingResidues(matchingRes);
				((CBSolvationCreator) energyCreators[6]).setMatchingResidues(matchingRes);
				((CBSolvationCreator) energyCreators[7]).setMatchingResidues(matchingRes);
				((CentroidSolvationCreator) energyCreators[8]).setMatchingResidues(matchingRes);
				((CentroidSolvationCreator) energyCreators[9]).setMatchingResidues(matchingRes);
				((CentroidSolvationCreator) energyCreators[10]).setMatchingResidues(matchingRes);
				((CentroidSolvationCreator) energyCreators[11]).setMatchingResidues(matchingRes);
				...
				// Analyzing residue energy
				E_ROT1_baysian_50[ali] = rot1_50.evaluate();
				for (int c=0 ; c<homology.atoms().size() ; c++) {
					participatingRes[homology.atoms().atomAt(c).residueNumber()] = true;
					resenergies[homology.atoms().atomAt(c).residueNumber()] += homology.atoms().atomAt(c).energy();
				}
				for (int c=0; c<participatingRes.length ; c++)
					if (participatingRes[c])
						numenergies[c]++;
				// Analyzing residue energy - End
			...
			for (int c=0; c<structuralHomology.residues().size() ; c++) {
				if (matchingRes[structuralHomology.residues().residueAt(c).number])
					System.out.println(structuralHomology.residues().residueAt(c).number + " 1 " +
						(resenergies[structuralHomology.residues().residueAt(c).number]/
						numenergies[structuralHomology.residues().residueAt(c).number]));
				else
					System.out.println(structuralHomology.residues().residueAt(c).number + " 0 " +
							(resenergies[structuralHomology.residues().residueAt(c).number]/
							numenergies[structuralHomology.residues().residueAt(c).number]));
			}
			System.exit(0);
			...
			for (int c=0 ; c<matchingRes.length ; c++) {
				double subsum = 0.0;
				for (int cc=c-3; cc<c+4 ; cc++)
					if ((cc>=0) && (cc<matchingRes.length))
							subsum += (resenergies[cc]/numenergies[cc]);
				if (subsum>0.0) {
						matchingRes[c] = false;
						System.out.println("888888 " + c);
				}
			}
...
//				matchingRes[c] = true;
//			for (int c=0 ; c<matchingRes.length ; c++) {
//				if ((resenergies[c]/numenergies[c])>0.3) {
//					if ((c-1)>=0)
//						matchingRes[c-1] = false;
//					if (c>=0)
//						matchingRes[c] = false;
//					if ((c+1)<matchingRes.length)
//						matchingRes[c+1] = false;
//				}
//			}
