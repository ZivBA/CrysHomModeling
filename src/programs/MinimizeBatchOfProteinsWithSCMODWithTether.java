...
public class MinimizeBatchOfProteinsWithSCMODWithTether extends MeshiProgram implements Residues, AtomTypes {
	...
	public static void main(String[] args) throws MinimizerException, LineSearchException {
		...
		EnergyCreator[] energyCreators = {
				...
				new TetherCreator(5.0,new AtomList.ClassNCaCFilter())
		};	
		...
		for (int i=0 ; i<models.length ; i++) { 		
			...
			if (model!=null) { 		
				try {
					...
					energy = new TotalEnergy(model, distanceMatrix, energyCreatorsForSCMODminimzation, commands);
					...
					model = buildModel(reference,models[i]);
					SCMOD.scmod(commands, lib, model, 3, 0.0, pp);
