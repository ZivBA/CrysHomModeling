...
public class FastaList extends SequenceList {
	...
	boolean empty;
		...
		if (!comment.equals("")) { 
		    ...
		    if (!sequence.endsWith("*")) { throw new RuntimeException(" A Fasta sequence mast end in *");}
		    add(new ResidueSequence(sequence.substring(0,sequence.length()-1), // to remove the last character '*'.
				     comment));
		}
	    }
	}
	catch (ResidueTypeException ex) {throw ex;}
	catch (Exception ex) {throw new RuntimeException("Bad Fasta file\n"+ex);}
    }
