...
public final class SolvateEnergy extends CooperativeEnergyTerm implements AtomTypes, Residues {
	...
	if (parameters.maxEnd > DistanceMatrix.rMax())
    ...
    private void characterizedDistance(Distance dis) {
    	...
    	// ----------------------------------------
	    sigmaValues.isDisInvolvesHydrogens = dis.atom1().isHydrogen || dis.atom2().isHydrogen;
    		
    	// Does this distance involves two polar atoms ??
    	// ----------------------------------------------
	    sigmaValues.isDisBetween2Polars = (dis.atom1().isOxygen || dis.atom1().isNitrogen || (dis.atom1().type == CSG)) &&
    ...
    public void inactivateFarFromAtom(Atom atom , double R) {
        ...
        for (int c=0; c<atomListSize ; c++) {
	        ...
	        inactive[c] = ((atom1.x() - x) > R) || ((atom1.x() - x) < -R) ||
