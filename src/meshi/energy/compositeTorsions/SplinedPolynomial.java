	...
	public double value( int derivVar, double ... args ) {
		...
		if( args.length == 1 ) {
			...
			if( derivVar == 0 )
				...
				for( i=0; i<4; i++ )
					result = result + 
								coefs[bin1x4+i]*quickPower(tor1,4-(i+1));
			else if( derivVar == 1 )
				/* derive first variable */
				for( i=0; i<4; i++ )
					result = result +
								coefs[bin1x4+i]*quickPower(tor1,3-(i+1))*(4-(i+1));
		}
				...
				for( i=0; i<4; i++ )
					for( j=0; j<4; j++ )
						result = result +
									...
									quickPower(tor1, 4-(i+1)) *
									quickPower(tor2, 4-(j+1));
			else if( derivVar == 1 )
