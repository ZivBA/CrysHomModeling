 ...
 * @author Nir
 */
class ExtractCoarseSolvateDataLocalMoves extends MeshiProgram implements Residues, AtomTypes, MeshiPotential, KeyWords, CompositeTorsionsDefinitions{
			...
			if (modelHardCopy.atoms().CAFilter().size() > min_Prot_Size) {
				...
				for (int res = 0; res < model.residues().size(); ) {
					...
					if (goodStretch) {
								...
								for (int c2 = 0; c2 < model.atoms().size(); c2++) {
									...
									if (!((atom1.residueNumber() >= compStart) && (atom1.residueNumber() < (compStart + compZone)) &&
										...
										if ((dis < cutoffNeighbor) &&
											...
											if (!atom2.isHydrogen &&
													(atom1.residueNumber() >= compStart) && (atom1.residueNumber() < (compStart + compZone)))
												attribute1.addToSumOfNeighborsROT1(1.0);
											if (!atom1.isHydrogen &&
													(atom2.residueNumber() >= compStart) && (atom2.residueNumber() < (compStart + compZone)))
												attribute2.addToSumOfNeighborsROT1(1.0);
											////CB sensing BB:
											//								if (atom2.isBackbone && !atom2.isHydrogen &&
											//										(atom1.residueNumber()>=compStart) && (atom1.residueNumber()<
											// (compStart+compZone)))
											//									attribute1.addToSumOfNeighbors(1.0);
											//								if (atom1.isBackbone && !atom1.isHydrogen &&
											//										(atom2.residueNumber()>=compStart) && (atom2.residueNumber()<
											// (compStart+compZone)))
											//									attribute2.addToSumOfNeighbors(1.0);
										}
									}
						...
						//							if ((dis<cutoffNeighbor) &&
						//									(Math.abs(dl.distanceAt(c).atom1().residueNumber() - dl.distanceAt(c).atom2()
						// .residueNumber())>ignore_local)) {
