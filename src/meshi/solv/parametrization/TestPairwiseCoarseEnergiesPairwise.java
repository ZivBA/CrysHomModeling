 ...
 * @author Nir
 */
class TestPairwiseCoarseEnergiesPairwise extends MeshiProgram
		implements Residues, AtomTypes, MeshiPotential, KeyWords, CompositeTorsionsDefinitions {
...
//				new CbPairwiseCreator(1.0)
		...
		for (String model1 : models) {
			...
			if (modelHardCopy.atoms().CAFilter().size() > min_Prot_Size) {
				...
				// Creating the energy
				dm = new DistanceMatrix(model.atoms(), 8.1, 1.0, 3);
				TotalEnergy energy = new TotalEnergy(model, dm, creators, commands);
				...
				for (int res = 0; res < model.residues().size(); ) {
					...
					if (goodStretch) {
						...
						//						if ((stretchStart<87) && (stretchStart+len-1>87)) {
						//							model.atoms().print(new MeshiWriter("1D4TA.S999.ROT1.pdb"));
						//							System.exit(1);
						//						}
	...
	private static void init(String[] args) {
		...
		double cutoffNeighbor = new Double(cutoffString);
