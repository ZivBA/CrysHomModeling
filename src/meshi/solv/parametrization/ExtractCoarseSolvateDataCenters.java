 ...
 * @author Nir
 */
class ExtractCoarseSolvateDataCenters extends MeshiProgram implements Residues, AtomTypes, MeshiPotential, KeyWords, CompositeTorsionsDefinitions{
   ...
   160, //w
   135}; //y 
/* Before modifications to Lys, Arg, Asp, Glu, Asn and Gln
        static double[] scSA = {20, //a
    	40, //c
    55, //d
    95, //e
   115, //f
					...
					if (goodStretch) {
								...
								for (int c2 = 0; c2 < model.atoms().size(); c2++) {
									...
									if (!((atom1.residueNumber() >= compStart) && (atom1.residueNumber() < (compStart + compZone)) &&
										...
										if ((dis < cutoffNeighbor) &&
											...
											attribute1 = (SolvateExtractionAttribute) atom1.getAttribute(MeshiAttribute
													.SOLVATE_EXTRACTION_ATTRIBUTE);
											attribute2 = (SolvateExtractionAttribute) atom2.getAttribute(MeshiAttribute
													.SOLVATE_EXTRACTION_ATTRIBUTE);
						...
						for (int c = 0; c < model.atoms().size(); c++) {
							...
							if ((atom.residueNumber() >= compStart) && (atom.residueNumber() < (compStart + compZone)) &&
								...
								dataCBs[atom.type][indNeighbor]++;
								indNeighbor = findBinIndice(binsNeighbor, attribute.getSumOfNeighborsROT1());
								dataROT1[atom.type][indNeighbor]++;
							} else {
	...
	private static void shiftThreadedProt(Protein prot, Protein protHardCopy, double[][] pp, double[][] ppHardCopy,
		...
		for (int cc=0 ; cc<len ; cc++) {
			...
			if ((prot.residue(inRes).type!=GLY) && (prot.residue(inRes).type!=ALA)) {
				...
				RotamericTools.putCBinSCcenter(prot.residue(inRes));
