 ...
 * @author Nir
 */
public class ExtractCoarseSolvateDataCenters extends MeshiProgram implements Residues, AtomTypes, MeshiPotential, KeyWords, CompositeTorsionsDefinitions{ 
   ...
   */
	...
	public static void main(String[] args) throws IOException{
		...
		for (int i=0 ; i<models.length ; i++) { 		
			...
			// Creating the model
			Protein modelHardCopy = new Protein(new AtomList(models[i]), new ResidueExtendedAtoms(ADD_ATOMS));
			if (modelHardCopy.atoms().CAFilter().size()>min_Prot_Size) {
				...
				Protein model = new Protein(new AtomList(models[i]), new ResidueExtendedAtoms(ADD_ATOMS));
