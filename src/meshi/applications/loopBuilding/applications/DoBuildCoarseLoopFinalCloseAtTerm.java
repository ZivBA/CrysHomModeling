...
import meshi.applications.loopBuilding.LoopBuilderFinalCloseAtTerm;
 ...
 * 
 */
public class DoBuildCoarseLoopFinalCloseAtTerm extends MeshiProgram implements Residues,
		AtomTypes { /**
 * The implemented
 ...
 **/
	...
	public static void main(String[] args) {
		...
		try {
			prot.atoms().print(new MeshiWriter(pathString+"/phase1/truncatedModel.pdb"));
		} catch (IOException e) {
		...
		LoopBuilderFinalCloseAtTerm loop = new LoopBuilderFinalCloseAtTerm(
	...
	protected static void init(String[] args) {
		...
		tmp = getOrderedArgument(args);
		if (tmp == null) throw new RuntimeException(errorMessage);
		loopType = (new Integer(tmp)).intValue();
		System.out.println("# Loop type is:"+ (new Integer(tmp)).intValue());
