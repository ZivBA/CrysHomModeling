...
//class CompleteClass implements Residues, AtomTypes, KeyWords {
//
//
// 	/**
...
//    	return complete(commands , holed , 1 , alignment.columnAt(alignment.size()-1).cell0().number);
//    }
//
//
// 	/**
...
// 	**/
//    private static Protein complete(CommandList commands, Protein holed, int firstToComplete, int lastToComplete) throws Exception {
//    	Sequence sequence1 = new PredictionResidueSequence(commands, TARGET_FILE_PATH);
//    	Sequence sequence2 = holed.sequence();
//    	SequenceAlignment alignment = SequenceAlignment.identityAlignment(sequence1, sequence2);
...
//    	int[] added = new int[alignment.columnAt(alignment.size()-1).cell0().number+1];
...
//	    			if (!alignment.getColumn(0,i).cell1().gap()) { // no need to complete
//    					for (int j=0 ; j<holed.residue(i).atoms().size() ; j++)
//    						newList.add(new Atom(holed.residue(i).atoms().atomAt(j)));
...
//   	}
//
//    private static int findNonDummy(Protein holed, int begin) {
