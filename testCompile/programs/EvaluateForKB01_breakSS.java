...
//				RotamericTools.correctNomenclature(model);
				fullMatchRefModel = getMatchingAtoms(reference, model.atoms()) != null;
				...
				// Breaking it into SS
				System.out.println("999999 " + i + " 5555 " + printSSbreakdown(reference, model, dssp));
...
//				model = new Protein(pathSecondEval + "/" + models[i]+"_KB-nH_bf.pdb", new ResidueExtendedAtoms(DO_NOT_ADD_ATOMS));
				...
				// Breaking it into SS
				System.out.println("999999 " + i + " 6666 " + printSSbreakdown(reference, model, dssp));
...
private static String printSSbreakdown(Protein reference, Protein model, DSSP dssp) {
	AtomList cas = reference.atoms().CAFilter();
	int numberOfResidues = cas.size();

	double numberOfHelix = 0.0;
	for (int c=0 ; c<numberOfResidues ; c++) {
		if (dssp.SSofRes(cas.atomAt(c).residueNumber(), ' ') == 'H') {
			numberOfHelix++;
		}
	}

	double numberOfSheet = 0.0;
	for (int c=0 ; c<numberOfResidues ; c++) {
		if (dssp.SSofRes(cas.atomAt(c).residueNumber(), ' ') == 'E') {
			numberOfSheet++;
		}
	}
	...
	alignedOfCoil = 100.0*alignedOfCoil/(0.0000001+numberOfCoil);	

	
	return (" " + numberOfHelix + " " + alignedOfHelix + " " + numberOfSheet + " " + alignedOfSheet + " " + 
			numberOfCoil + " " + alignedOfCoil + " ");
}
