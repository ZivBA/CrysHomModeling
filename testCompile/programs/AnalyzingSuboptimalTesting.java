...
class AnalyzingSuboptimalTesting extends MeshiProgram implements Residues,
	...
	public static void main(String[] args) {
		...
		int findInd = 62;
		for (int ali = findInd; ali < findInd + 1; ali++) {
			System.out.println("\n******************\n" + ali + "\n******************");
			Protein homology;
			...
			try {
				homology.atoms().print(new MeshiWriter(dirName + "/" + (findInd) + ".pdb"));
			} catch (Exception ignored) {
			}
			...
			cb_80 = (ROT1SolvationEnergy) (energy.getEnergyTerms(new ROT1SolvationEnergy())[11]);
			rot1_80.evaluate();
