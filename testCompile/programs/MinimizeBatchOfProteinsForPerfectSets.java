...
class MinimizeBatchOfProteinsForPerfectSets extends MeshiProgram implements Residues, AtomTypes {
	...
	for (int i=0 ; i<models.length ; i++) {
		// Reading the model
		model = new Protein(getMatchingAtoms(reference.atoms().noOXTFilter(),
				new AtomList(models[i])) , new ResidueExtendedAtoms(DO_NOT_ADD_ATOMS));
	...
	private static AtomList getMatchingAtoms(AtomList refAtoms , AtomList protAtoms) {
		...
		try {
			...
			System.out.println("\n\nThe reference atom:\n" + refAtoms.atomAt(c) + "\nIs not in the model. This should not be!\n\n");
