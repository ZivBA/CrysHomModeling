...
public class IsiteLib extends Vector<Imotif> implements Residues {
	...
	private final static double veryHighEnergy = 100000000000.0;
	...
	public IsiteLib(String fileName) {
		...
		try{
			...
			do {
				...
				Vector<String> libString = new Vector<>();
				libString.add(line);
				line = br.readLine();
				...
				double[][] confCurvIsite51 = {{10034, -0.709541, 5.390276},
				...
				double[][] zScoresIsite51 = {{10034, -6.847607, 3.517556},
	...
	private int findCenterOfMotif(Imotif motif) {
...
//					System.out.println(0 + " " + calcScore(motif, corpus, corpus.ungapped[c]));
	...
	private static boolean isFragInMotif(Imotif motif, Corpus corpus, int centerInCorpus, int motifStart) {
...
//		if ((mda<dmaCut) && (dme<dmeCut)) {
		return (mda < Math.min(Math.PI / 6.0 + motif.getMotifLength() / 15.0 * Math.PI / 6.0, dmaCut)) && (dme < Math.min(dmeCut,
				1.5 + 0.25 * (motif.getMotifLength() - 8)));
	}
...
//								" [" + fmt.format(overturnScore) + " " + fmt.format(overturnmdas) + " " + fmt.format(overturndmes) + "]");
	...
	public void loadAndAnalyzeSequenceIntoLib(int[] seq) {
		...
		for (int motifInd=0 ; motifInd<size() ; motifInd++) {
			...
			for (int c=0 ; c<loadedSeq.length-2*overhang-motifLength+1 ; c++) {
				System.arraycopy(loadedSeq, c + 0, tmpSeq, 0, 2 * overhang + motifLength);
	...
	private double[] negativeScoresOfMotifInSeqPos(int pos) {
	...
	private static double calcScore(Imotif motif, Corpus corpus, int seqStart) {
		int[] seq = new int[motif.getMotifLength() + 2*motif.getOverhang()];
		System.arraycopy(corpus.resType, seqStart + 0, seq, 0, motif.getMotifLength() + 2 * motif.getOverhang());
...
//	}
	...
	private static double calcScoreFromSeq(Imotif motif, int[] seq) {
		double pScore = 0.0;
		double[] bkProb = {0.0749, 0.0182, 0.0522, 0.0626, 0.0391, 0.071, 0.0223, 0.0545, 0.0582, 0.0906, 0.0227, 0.0453, 0.0512, 0.0411, 0.0522, 0.0734, 0.0596, 0.0648, 0.0132, 0.0325};
		double alpha = 0.15;
		int overhang = motif.getOverhang();
...
//		return confScore;
	...
	private static double calcConfFromScore(Imotif motif, double pScore) {
		return 1/(1+Math.exp(motif.confidenceA()*pScore+motif.confidenceB())) + 0.0*motif.getMotifLength();
	}

	
	private static double calcMDA(Imotif motif, Corpus corpus, int anglesStart) {
		double maxDMA = -1;
	...
	private static double calcDME(Imotif motif, Corpus corpus, int center, int fragStart) {
