 ...
 * @author Nir
 */
class TestPairwiseCoarseEnergiesCenters extends MeshiProgram implements Residues, AtomTypes, MeshiPotential, KeyWords, CompositeTorsionsDefinitions{
	...
	// Constants:
	// ------------------------
    private static final double[] scSA = {20, //a
    	40, //c
        45, //d
        80, //e
       115, //f
       ...
       160, //w
       135}; //y 
    private static final double bbSA = 55.0/75.0;  // Should be 95,  but this is without hydrogen bonding in a SS;
	...
	public static void main(String[] args) throws IOException{
		...
		for (String model1 : models) {
			System.out.println("Reading: " + prefixName + model1 + suffixName);
			
			// Creating the model
			Protein modelHardCopy = new Protein(new AtomList(prefixName + model1 + suffixName), new ResidueExtendedAtoms(ADD_ATOMS));
			if (modelHardCopy.atoms().CAFilter().size() > min_Prot_Size) {
				...
				Protein model = new Protein(new AtomList(prefixName + model1 + suffixName), new ResidueExtendedAtoms(ADD_ATOMS));
				...
				for (int res = 0; res < modelHardCopy.residues().size(); res++) {
					if ((modelHardCopy.residues().residueAt(res).type < 20) &&
						...
						RotamericTools.putCBinSCcenter(modelHardCopy.residues().residueAt(res));
				}
				for (int res = 0; res < model.residues().size(); res++) {
					if ((model.residues().residueAt(res).type < 20) &&
							(model.residues().residueAt(res).type > -1))
						RotamericTools.putCBinSCcenter(model.residues().residueAt(res));
				}
				...
				for (int res = 0; res < model.residues().size(); ) {
					...
					if (goodStretch) {
						...
						//						if ((compStart<=116) && (compStart+compZone>116)) {
						//							model.atoms().print(new MeshiWriter("1IKPA.S0.ROT1.pdb"));
						//							System.exit(1);
						//						}
						...
						for (int c = 0; c < dl.size(); c++) {
							...
							if ((dis < cutoffNeighbor) &&
								...
								double factor = 36.0 / Math.max(3.0, dis) * 1.0 / Math.max(3.0, dis);
								if (atom2.name.equals("CB") &&
										(atom1.residueNumber() >= compStart) && (atom1.residueNumber() < (compStart + compZone)))
									attribute1.addToSumOfNeighbors(factor * scSA[atom2.residue().type] / 75.0);
								if (atom1.name.equals("CB") &&
										(atom2.residueNumber() >= compStart) && (atom2.residueNumber() < (compStart + compZone)))
									attribute2.addToSumOfNeighbors(factor * scSA[atom1.residue().type] / 75.0);
								if (atom2.name.equals("CA") &&
										(atom1.residueNumber() >= compStart) && (atom1.residueNumber() < (compStart + compZone)))
									attribute1.addToSumOfNeighbors(factor * bbSA);
								if (atom1.name.equals("CA") &&
										(atom2.residueNumber() >= compStart) && (atom2.residueNumber() < (compStart + compZone)))
									attribute2.addToSumOfNeighbors(factor * bbSA);
								if (atom2.name.equals("CB") &&
										(atom1.residueNumber() >= compStart) && (atom1.residueNumber() < (compStart + compZone)))
									attribute1.addToSumOfNeighborsROT1(factor * scSA[atom2.residue().type] / 75.0);
								if (atom1.name.equals("CB") &&
										(atom2.residueNumber() >= compStart) && (atom2.residueNumber() < (compStart + compZone)))
									attribute2.addToSumOfNeighborsROT1(factor * scSA[atom1.residue().type] / 75.0);
								if (atom2.name.equals("CA") &&
										(atom1.residueNumber() >= compStart) && (atom1.residueNumber() < (compStart + compZone)))
									attribute1.addToSumOfNeighborsROT1(factor * bbSA);
								if (atom1.name.equals("CA") &&
										(atom2.residueNumber() >= compStart) && (atom2.residueNumber() < (compStart + compZone)))
									attribute2.addToSumOfNeighborsROT1(factor * bbSA);
								//								if (((compStart<116) && (compStart+compZone>116)) &&
								//										(atom1.name.equals("CB") && (atom1.residueNumber()==116) &&
								//												(atom2.name.equals("CB")||atom2.name.equals("CA")))) {
								//									if (atom2.name.equals("CA"))
								//									System.out.println(atom1 +"\n"+atom2+"\n"+bbSA+"\n");
								//									else
								//									System.out.println(atom1 +"\n"+atom2+"\n"+(scSA[atom2.residue().type]/75.0)
								// +"\n");
								//								}
								//								if (((compStart<116) && (compStart+compZone>116)) &&
								//										(atom2.name.equals("CB") && (atom2.residueNumber()==116) &&
								//												(atom1.name.equals("CB")||atom1.name.equals("CA")))) {
								//									if (atom1.name.equals("CA"))
								//										System.out.println(atom2 +"\n"+atom1+"\n"+bbSA+"\n");
								//										else
								//										System.out.println(atom2 +"\n"+atom1+"\n"+(scSA[atom1.residue().type]/75.0)
								// +"\n");
								//								}
						...
						//						if ((stretchStart<126) && (stretchStart+len-1>126)) {
						//							System.exit(1);
						//						}
						...
						for (int c = 0; c < model.atoms().size(); c++) {
							...
							if ((atom.residueNumber() >= compStart) && (atom.residueNumber() < (compStart + compZone)) &&
								...
								if (!atom.isBackbone || atom.name().equals("CB")) {
									System.out.println(
											"XXXXX " + model1 + " " + atom.residueNumber() + " " + atom.residueName() + " " + atom.name + " " +
													attribute.getSumOfNeighbors());
								}
								if (!atom.isBackbone || atom.name().equals("CB")) {
									System.out.println(
											"YYYYY " + model1 + " " + atom.residueNumber() + " " + atom.residueName() + " " + atom.name + " " +
	...
	private static void shiftThreadedProt(Protein prot, Protein protHardCopy, double[][] pp, double[][] ppHardCopy,
		...
		for (int cc=0 ; cc<len ; cc++) {
			...
			if ((prot.residue(inRes).type!=GLY) && (prot.residue(inRes).type!=ALA)) {
				...
				RotamericTools.putCBinSCcenter(prot.residue(inRes)); // This is for centroid
// 				ResidueBuilder.build(prot.residue(inRes), (int) pp[inRes][2], 
